...
const actions = {
  // 筛选用户所拥有的权限路由（动态路由）
  // 第二个参数为当前用户的所拥有的菜单权限
  // 用户资料里面的menus: ["settings","permissions"]
  // asyncRoutes是所有的动态路由
  ...
  filterRoutes(context, menus) {
    ...
    menus.forEach(key => {
      ...
      // 在所有动态路由中筛选出 路由里面 name属性值 和 用户资料中menus属性（用户所拥有的权限）值(所拥有的权限的权限标识)  相同的
      routes.push(...asyncRoutes.filter(item => item.name === key)) // 得到一个数组 有可能 有元素 也有可能是空数组
    })
    // 得到的routes是所有模块中满足权限要求的动态路由数组
    // routes就是当前用户所拥有的 动态路由的权限
    context.commit('setRoutes', routes)
    return routes // state中的routes是用来显示左侧菜单 而 return是用来addRoutes动态添加有权限的动态路由 到路由表里
  }
